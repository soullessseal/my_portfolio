...
type DetailContent = {
  ...
  goals: string[];
  strategies: Array<{ imageSrc: string; text: string }>;
  processFlowItems?: Array<{ imageSrc: string; label: string }>;
};

const DEFAULT_SWISS_CONTENT: DetailContent = {
  englishTitle: "SWITZERLAND TRAVEL PROJECT",
  title: "瑞士旅遊專案",
  heroImageSrc: "/figma-assets/feature-a.png",
  projectType: "平面視覺設計",
  projectBackground:
  ...
  strategies: [
    {
      imageSrc: "/figma-assets/strategy-a1.png",
      text: "以較豐富的色彩層次，提升旅遊畫面的吸引力",
    },
    {
      imageSrc: "/figma-assets/strategy-a2.png",
      text: "主色採天空藍與瑞士紅，輔以自然綠與雪白",
    },
    {
      imageSrc: "/figma-assets/strategy-a3.png",
...
const INDIA_CONTENT: DetailContent = {
  ...
  title: "印度朝聖海外網專案",
  heroImageSrc: "/figma-assets/feature-c.png",
...
const PORTFOLIO_CONTENT: DetailContent = {
  ...
  title: "個人作品集網站",
  heroImageSrc: "/figma-assets/feature-b.png",
  ...
  strategies: [
    {
      imageSrc: "/figma-assets/strategy-b1.png",
      text: "採用簡潔且低彩度的視覺風格，以米色與灰階色系建立沉穩且一致的網站調性，避免過度干擾作品內容",
    },
    {
      imageSrc: "/figma-assets/strategy-b2.png",
      text: "規劃清楚的作品分類（UI/UX、平面設計、其他），讓瀏覽者可以快速找到感興趣的作品類型",
    },
    {
      imageSrc: "/figma-assets/strategy-b3.png",
      text: "透過卡片式作品版型設計，讓不同專案在視覺上保持一致性，同時方便後續新增與擴充作品",
    },
    {
      imageSrc: "/figma-assets/strategy-b4.png",
      text: "採用響應式設計（RWD），確保網站在桌機與手機裝置上皆能維持良好的閱讀體驗",
    },
  ],
  processFlowItems: [
    { imageSrc: "/figma-assets/step-1.png", label: "分析需求並繪製網站心智圖(Mind Map)" },
    { imageSrc: "/figma-assets/step-2.png", label: "整理內容層級並規劃網站地圖(Site Map)" },
    { imageSrc: "/figma-assets/step-3.png", label: "繪製線框稿架構(Wireframe)" },
    { imageSrc: "/figma-assets/step-4.png", label: "製作元件/變體(Components/Varients)，建立UI System(元件系統規範)" },
    { imageSrc: "/figma-assets/step-5.png", label: "串接設計(Figma MCP)與AI程式模型(Codex)，以及後台Sanity(CMS)" },
  ],
};
...
export default function FeaturedDetailModal({ project, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      goalsTimeline.to(
        ...
        { y: -24, autoAlpha: 0, ease: "power2.in", duration: 0.35 },
        ">+0.1",
      );
      goalsTimeline.to(
        goalCards,
      ...
      goalsTimeline.to(
        goalsContentRef.current,
        { y: -72, autoAlpha: 0, ease: "power2.in", duration: 0.45 },
        "cardsGone+=0.25",
      );
      gsap.fromTo(
      ...
      strategyTimeline
        .to({}, { duration: 0.08 })
        .to(strategyTrackRef.current, { xPercent: strategyMaxShiftPercent, ease: "none", duration: 0.86 })
        .to({}, { duration: 0.08 });
      ...
      if (
        ...
        const processItemCount = detail.processFlowItems.length;
        const processTimeline = gsap.timeline({
          scrollTrigger: {
            trigger: processFlowRef.current,
            start: "top top",
            end: isMobileViewport
              ? `+=${Math.round(145 * Math.max(1, (processItemCount - 1) / 2))}%`
              : `+=${Math.round(130 * Math.max(1, (processItemCount - 1) / 2))}%`,
            pin: true,
            scrub: isMobileViewport ? 1.1 : 1.2,
          },
        });
        ...
        processTimeline
          ...
          .to(
            ...
            {
              x: () => {
                const track = processFlowTrackRef.current;
                const section = processFlowRef.current;
                ...
                if (!viewport || !lastItem) return -(track.scrollWidth - section.clientWidth);

                const viewportWidth = viewport.clientWidth;
                const lastCenter = lastItem.offsetLeft + lastItem.offsetWidth / 2;
                const centeredX = -(lastCenter - viewportWidth / 2);
                const minX = viewportWidth - track.scrollWidth;

                return Math.max(minX, Math.min(0, centeredX));
              },
              ease: "none",
              duration: 0.86,
            },
        ...
        <div className="relative h-full w-full overflow-hidden rounded-t-[22px] rounded-b-none border border-secondary-50 border-b-0 bg-primary shadow-[0px_0px_8px_var(--color-word1-50)] md:rounded-t-[26px]">
          ...
          <div ref={wrapperRef} className="absolute inset-0 overflow-hidden [contain:layout_paint] [clip-path:inset(56px_0_0_0)] md:[clip-path:inset(80px_0_0_0)]">
            <div ref={contentRef} className="w-full min-h-full">
              <div className="w-full" data-name="Featured_GSAP_Content_Wrapper">
                <section
                  ref={heroRef}
                  className="relative min-h-screen w-full px-[16px] pb-[16px] pt-[96px] md:px-[24px] md:pb-[20px] md:pt-[120px]"
                >
                  <div className="relative z-20 mx-auto flex w-full max-w-[1440px] flex-col items-center">
                    <div ref={heroTitleRef} className="mt-[20px] flex flex-col items-center text-center md:mt-[28px]">
                      ...
                      <button
                        type="button"
                        className="mt-[28px] inline-flex items-center gap-[8px] rounded-[999px] border border-highlight bg-highlight px-[18px] py-[10px] text-h3 text-primary md:mt-[44px]"
                      >
                        向下滑動 <span aria-hidden="true">↓</span>
                      </button>
                    </div>

                    <div
                      ref={heroImageRef}
                      className="self-center mt-[16px] h-[100dvh] w-[200vw] min-w-0 overflow-hidden md:self-start md:h-[100dvh] md:w-full"
                    >
                      <Image
                        src={detail.heroImageSrc}
                        alt={`${detail.title} Hero 圖`}
                        width={2200}
                        height={1240}
                        sizes="100vw"
                  ...
                  <div
                    ref={heroDetailRef}
                    className="relative z-30 mx-auto -mt-[56px] w-full max-w-full rounded-[14px] bg-[var(--color-primary-85)] p-[10px] md:absolute md:left-[calc(62%_+_32px)] md:top-[61%] md:mt-0 md:max-h-[44%] md:w-[clamp(260px,32vw,520px)] md:max-w-none md:-translate-y-1/2 md:overflow-y-auto md:p-[14px]"
                  >
                ...
                <section
                  ref={goalsRef}
                  className="h-screen w-full overflow-hidden bg-highlight px-[16px] pb-[24px] pt-[80px] md:px-[24px] md:pb-[30px] md:pt-[110px]"
                >
                  <div ref={goalsContentRef} className="mx-auto flex h-full w-full max-w-[1440px] flex-col">
                    <h3 ref={goalsTitleRef} className="mt-[56px] text-gsap-section-title text-center text-primary md:mt-[72px]">設計目標</h3>
                    <div className="mt-[18px] grid h-full min-h-0 w-full min-w-0 flex-1 grid-cols-1 items-center gap-[28px] md:grid-cols-3 md:gap-[16px]">
                      {detail.goals.map((goal, index) => (
                        <article
                          key={goal}
                          data-goal-card
                          className="flex h-full w-[88%] min-w-0 justify-self-center flex-col items-center justify-center gap-[16px] overflow-hidden rounded-[14px] border-[3px] border-primary bg-[var(--color-primary-85)] p-[10px] text-center shadow-[0px_10px_24px_var(--color-black-50)] md:h-full md:max-h-[340px] md:w-full md:gap-[20px] md:p-[14px]"
                        >
                          <p className="text-gsap-goal-number text-highlight">{`0${index + 1}`}</p>
                          <p className="mx-auto text-center text-[16px] leading-[1.35] text-word2 md:text-[clamp(20px,1.8vw,28px)]">
                            {goal}
                          </p>
                        </article>
                      ))}
                ...
                <section
                  ref={strategyRef}
                  className="h-screen w-full overflow-hidden px-[16px] pb-[24px] pt-[80px] md:px-[24px] md:pb-[30px] md:pt-[110px]"
                >
                  <div ref={strategyContentRef} className="relative h-full w-full min-w-0">
                    <h3 className="pointer-events-none absolute left-1/2 top-[56px] z-20 -translate-x-1/2 text-gsap-section-title text-word2 md:top-[72px]">
                      設計策略
                    </h3>
                    <div className="relative flex h-full w-full min-w-0 items-start overflow-hidden pt-[96px] md:pt-[128px]">
                      <div
                        ...
                        style={{ width: `${detail.strategies.length * 100}vw` }}
                      >
                        {detail.strategies.map((item) => (
                          <article key={item.text} className="flex h-full w-screen min-w-0 shrink-0 items-center justify-center px-[12px] md:px-[20px]">
                            <div className="mx-auto grid h-[min(64vh,100%)] w-full max-w-[1360px] grid-cols-1 items-center gap-[18px] md:grid-cols-2 md:gap-[56px]">
                              ...
                              <div className="w-full max-w-[85vw] min-w-0 md:max-w-none">
                                <p
                                  data-strategy-text
                                  className="min-w-0 text-[16px] leading-[1.35] text-word2 md:text-[clamp(20px,1.8vw,28px)]"
                                >
                                  {item.text}
                                </p>
                              </div>
                      ...
                      <div className="relative flex h-full w-full min-w-0 items-start overflow-hidden pt-[96px] md:pt-[128px]">
                        ...
                        >
                          {detail.processFlowItems.map((item, index) => (
                            <article
                              key={item.label}
                              className="flex h-full w-[calc(100vw-32px)] min-w-[calc(100vw-32px)] shrink-0 items-center justify-center"
                            >
                              <div className="mx-auto flex h-[min(64vh,100%)] w-full max-w-[980px] flex-col items-center justify-center gap-[10px] md:gap-[14px]">
                                ...
                                <p
                                  data-process-text
                                  className="min-w-0 text-center text-[16px] leading-[1.35] text-word2 md:text-[clamp(20px,1.8vw,28px)]"
                                >
                ...
                <section className="w-full bg-highlight px-[16px] py-[20px] md:px-[24px] md:py-[24px]">
                  <div className="mx-auto w-full max-w-[1440px] text-center">
                    <Link
                      href="/page-about"
                      onClick={onClose}
                      className="group inline-flex items-center gap-[8px] text-h4 text-primary transition-all duration-150 hover:text-word2 hover:opacity-100 active:scale-[0.97] active:text-word2 active:opacity-100"
                    >
                      DESIGN BY BETTY CHOU{" "}
                      <span
                        aria-hidden="true"
                        className="transition-transform duration-150 group-hover:translate-x-[4px] group-active:translate-x-[2px]"
                      >
                        →
                      </span>
                    </Link>
